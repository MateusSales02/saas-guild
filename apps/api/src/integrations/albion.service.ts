...
export class AlbionService {
  private readonly statusUrl =
    process.env.ALBION_STATUS_URL ??
    'https://gameinfo.albiononline.com/api/gameinfo/servers/status';
  private readonly activitiesUrl =
    process.env.ALBION_ACTIVITIES_URL ??
  ...
  async fetchActivities(limit = 5): Promise<AlbionActivity[]> {
    ...
    if (!res.ok) {
      this.logger.warn(
        `Falha ao consultar atividades do Albion: ${res.status}`,
      );
